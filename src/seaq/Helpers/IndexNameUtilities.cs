    ...
    public static class IndexNameUtilities
    {
            ...
            this string indexName)
        {
            ...
            if (!indexName.Equals(indexName.ToLowerInvariant()))
            {
                ...
                Log.Debug("Provided index name ({0}) was not all lowercase as required - index name coerced to {1}", oldName, indexName);
            }
            return indexName;
        }
            ...
            string clusterScope)
        {
            if (!indexName.StartsWith(clusterScope, StringComparison.OrdinalIgnoreCase))
            {
                ...
                Log.Debug("Provided index name ({0}) did not begin with cluster's scope as required - index name coerced to {1}", oldName, indexName);
            }
            return indexName;
        }
    }
